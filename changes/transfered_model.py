...
class TransformMSNComplex(nn.Module):
    def __init__(self, residual, additional_encoder, additional_decoder, transform2d,
        ...
        self.decoder = nn.ModuleList(
            [PointGenCon(transformf=self.transformDecoder, bottleneck_size=2 + self.bottleneck_size) for i in
             range(0, self.n_primitives)])
        self.additional_encoder = additional_encoder(self.bottleneck_size)
        self.additional_decoder = additional_decoder(3, (self.bottleneck_size + 2) // 2,
                                                     (self.bottleneck_size + 2) // 4, self.bottleneck_size)
        self.res = residual()
        self.expansion = expansion.expansionPenaltyModule()
